...
define( 'BMFK_THEME_VERSION', '1.6.14' );
...
/**
 * Add valid root rewrite rules for SuperPWA's generated manifest and service
 * worker. SuperPWA 2.2.46 prefixes its regular expressions with a slash,
 * which does not match WordPress' root rewrite path on the current web hotel.
 */
function bmfk_add_superpwa_rewrite_compatibility() {
	if ( ! function_exists( 'superpwa_get_manifest_filename' ) || ! function_exists( 'superpwa_get_sw_filename' ) ) {
		return;
	}
	...
	add_rewrite_rule(
		'^' . preg_quote( $manifest_filename, '/' ) . '$',
		'index.php?pagename=' . $manifest_filename,
		'top'
	);
	add_rewrite_rule(
		'^' . preg_quote( $sw_filename, '/' ) . '$',
		'index.php?pagename=' . $sw_filename,
		'top'
	);
}
add_action( 'init', 'bmfk_add_superpwa_rewrite_compatibility', 20 );

/**
 * Flush rewrite rules once after the compatibility rules become available.
 * This only runs in wp-admin and only when SuperPWA is active.
 */
function bmfk_activate_superpwa_rewrite_compatibility() {
	if ( ! function_exists( 'superpwa_get_manifest_filename' ) || ! function_exists( 'superpwa_get_sw_filename' ) ) {
		return;
	}

	if ( '1.6.3' === get_option( 'bmfk_superpwa_rewrite_version' ) ) {
		return;
	}

	flush_rewrite_rules( false );
	update_option( 'bmfk_superpwa_rewrite_version', '1.6.3', false );
}
add_action( 'admin_init', 'bmfk_activate_superpwa_rewrite_compatibility' );

/**
 * Keep SuperPWA's generated JavaScript valid when its exclusion textarea has
 * retained a line break, and always keep the live webcam response out of the
 * service-worker cache.
 *
 * @param string $urls JavaScript regular expressions generated by SuperPWA.
 * @return string
 */
function bmfk_superpwa_never_cache_urls( $urls ) {
	$urls = str_replace( array( "\r", "\n" ), '', (string) $urls );

	if ( false === strpos( $urls, 'bmfk_webcam=1' ) ) {
		$urls .= ',/bmfk_webcam=1/';
	}

	return $urls;
}
add_filter( 'superpwa_sw_never_cache_urls', 'bmfk_superpwa_never_cache_urls', 10000 );
