Velkommen til Bodø Modellflyklubb! Her finner du det viktigste du trenger for å komme trygt og enkelt i gang med klubben, flyplassen på Bestemorenga og Norges Luftsportforbund (NLF).

> **Begynn rolig:** Er du ny i klubben eller på flyplassen, avtal den første turen med en instruktør eller et erfarent medlem. Ikke fly alene før du kjenner lokale regler, flysoner og rutinene på plassen.

[Button: Last ned velkomstguiden som PDF]({{new_member_pdf_url}})

## Start her - sju viktige punkter

- Kontroller at medlemskapet og kontingenten er registrert i [Min idrett](https://www.minidrett.no/).
- Bli med i [Facebook-gruppen for medlemmer]({{facebook_members_url}}), der klubben deler aktiviteter og praktiske beskjeder.
- Les [flyplassreglene]({{rules_url}}) og gjeldende [Modellflyhåndbok]({{handbook_url}}) før du flyr.
- Avtal introduksjon, opplæring eller kontroll av modellen med en instruktør eller erfaren klubbkamerat.
- Kontroller operatørregistreringen din og merk modellene med riktig operatørnummer.
- Sjekk [værstasjon og webkamera]({{webcam_url}}) før du kjører til Bestemorenga.
- Spør når du er usikker. Det er både tryggere og raskere enn å gjette.

## Klubben, NLF, Min idrett og TMS

:::columns
:::column
## Bodø Modellflyklubb

Klubben driver modellflyplassen og klubbhytta på Bestemorenga. Her får du lokalkunnskap, opplæring, et sosialt miljø og hjelp med modellfly, helikopter, droner og FPV.

:::column
## Norges Luftsportforbund

NLF Modellflyseksjonen forvalter Modellflyhåndboka, kompetanseordningen, ansvarsforsikringen, sikkerhetsinformasjon og hendelsesrapportering for organisert modellflyging.
:::endcolumns

:::columns
:::column
## Min idrett

[Min idrett](https://www.minidrett.no/) er medlemsportalen til Norges idrettsforbund. Der kontrollerer du kontaktopplysninger, klubbmedlemskap og fakturaer. Hold e-postadresse og mobilnummer oppdatert.

:::column
## TMS

[TMS](https://tms.nlf.no/) er NLFs løsning for kompetanse. Der finner du kompetansebevis, søker om ny kompetanse og fornyer bevis når vilkårene er oppfylt. Du logger inn med samme idrettsbruker som i Min idrett.
:::endcolumns

## Medlemskap og betaling

Du melder deg inn gjennom klubbens innmeldingsside. Medlemsfordeler og forsikringsdekning gjelder når riktig medlemskap og kontingent er registrert betalt.

[Button: Åpne innmeldingen]({{membership_url}})

Hvis medlemskap, kategori eller faktura ser feil ut, bør du ikke endre beløpet selv. Kontroller opplysningene i Min idrett og kontakt klubben eller [NLFs medlemsservice](https://nlf.no/grener/modellfly/modellflyseksjonen/kontakt-modellflyseksjonen/) før betaling.

## Opplæring og kompetanse

En instruktør hjelper deg med teori, sikker bruk av utstyret, lokale bestemmelser og praktisk flyging. Hvilken kompetanse du trenger avhenger av modelltype og aktivitet. A-bevis er inngangen til selvstendig modellflyging for mange, mens blant annet stormodell, turbin og enkelte FPV-aktiviteter har egne krav.

Søknad om ny kompetanse gjøres av deg i TMS og bekreftes digitalt av instruktør eller klubbleder når det kreves. Kontroller alltid gjeldende krav i Modellflyhåndboka og hos NLF.

:::columns
:::column
## Før du flyr alene

- Gjennomfør nødvendig teori og praktisk opplæring.
- Lær flysoner, pilotområde og lokale varslingsrutiner.
- Få modellen og radiooppsettet kontrollert dersom du er usikker.
- Kontroller at kompetansen din er synlig og gyldig i TMS.

:::column
## Trenger du instruktør?

Kontakt klubben eller se oversikten over [kontaktpersoner og ansvarlige]({{group_contacts_url}}). Fortell hva du ønsker å fly og hvor mye erfaring du har, så finner klubben riktig person.
:::endcolumns

## FPV - sjekkliste og teoriprøve

Ved FPV-flyging gjelder egne bestemmelser i Modellflyhåndboka. Skal du fly FPV uten utkikksperson, krever NLF at den digitale sjekklisten fylles ut før hver flyging. Sjekklisten kan åpnes og fylles ut på mobil.

NLFs teoriprøve for FPV uten utkikksperson er tilgjengelig i TMS. NLF har varslet at bestått prøve blir et krav, og anbefaler at den gjennomføres allerede nå. Kontroller alltid gjeldende status i TMS og Modellflyhåndboka før du flyr.

- [Åpne NLFs digitale FPV-sjekkliste](https://nlf.no/grener/modellfly/sikkerhet-utdanning/sjekkliste-fpv/)
- [Les NLFs informasjon om FPV-teoriprøven](https://nlf.no/Nyhetsvisning/2026/ta-teoriprove-for-fpv-i-dag-sa-er-du-klar-nar-kravet-kommer/)

## Ansvarsforsikring

Aktive medlemmer i NLF Modellflyseksjonen er omfattet av den kollektive ansvarsforsikringen. Støttemedlemskap er ikke omfattet. Forsikringen gjelder ansvarsskade på tredjeperson som følge av flyging; den erstatter ikke skade, tap eller tyveri av din egen modell.

Forsikring betyr ikke at regler kan fravikes. Ulovlig eller grovt uaktsom flyging kan få alvorlige følger, blant annet krav om tilbakebetaling fra forsikringsselskapet. Les alltid [gjeldende forsikringsinformasjon hos NLF](https://nlf.no/grener/modellfly/forsikring/sporsmal-og-svar-om-forsikring/).

## Operatørregistrering og merking

NLF tilbyr automatisk registrering i Luftfartstilsynets operatørregister for aktive modellflymedlemmer. Registreringen står som en egen linje på kontingentfakturaen og gjennomføres når betalingen er registrert. Du kan kontrollere statusen i TMS.

- Operatørnummeret tilhører deg, ikke den enkelte modellen.
- Det samme nummeret brukes på modellfly, helikopter, multirotor og droner du opererer.
- Modellene skal merkes etter gjeldende regler. De tre siste tegnene i operatørnøkkelen er hemmelige og skal ikke stå synlig på modellen.
- A1/A3-kompetanse fra flydrone.no er ikke nødvendig når du flyr i regi av BMFK/NLF og under Modellflyhåndboka.

[Les NLFs oppdaterte informasjon om operatørregistrering →](https://nlf.no/grener/modellfly/sikkerhet-utdanning/registrering-i-droneoperatorregisteret/)

> **Hvilke regler flyr du etter?** Som medlem som flyr i regi av BMFK/NLF, følger du Modellflyhåndboka, klubbens lokale regler og kompetansekravene i NLFs godkjente sikkerhetssystem. Flyging utenfor dette systemet må skje i åpen kategori eller under en annen gyldig tillatelse. Operatørregistrering og merking er egne krav. [Les hele forklaringen under Flyplassregler.]({{rules_url}})

## Bestemorenga modellflyplass

Selve Bestemorenga modellflyplass ligger rett utenfor femkilometersonen rundt Bodø lufthavn. Flyging vest for rullebanen kan likevel berøre sonen. Klubbens avtale med Bodø kontrolltårn gjelder flyging i regi av Bodø Modellflyklubb og under NLFs sikkerhetssystem.

[Button: Les lokale regler og avtalen med kontrolltårnet]({{rules_url}})

### Gjeldende åpningstider

- Elektromotor: **{{electric_hours}}**
- Forbrenningsmotor: **{{combustion_hours}}**
- Klubbhytta: **hver dag kl. 08:00-00:00**

Oppslag på plassen, sikkerhetsbeskjeder og instrukser fra klubbens ansvarlige går alltid foran denne korte oversikten.

## Første flydag - praktisk sjekkliste

- Avtal gjerne tidspunkt med et annet medlem eller en instruktør.
- Sjekk vind, vær, sikt og kamerabildet fra Bestemorenga.
- Ta med modell, sender, ladede batterier, lader, nødvendige verktøy og klær for været.
- Kontroller modell, rorretninger, propell eller rotor, batterifeste, rekkevidde og failsafe etter riktig prosedyre.
- Koble til fremdriftsbatteri eller start motor bare på anvist og sikker plass.
- Avklar aktiv rullebane og flyretning med de andre pilotene, og varsle tydelig før avgang og landing.
- Stans dersom mennesker, kjøretøy, bemannet luftfart eller andre forhold gjør flygingen usikker.
- Rydd etter deg og ta skadde batterier og annet avfall med hjem.

> En sjekkliste erstatter ikke opplæring. Følg alltid instruktøren, lokale regler og Modellflyhåndboka.

## Klubbhytta og adgang

Klubbhytta er samlingspunktet vårt på Bestemorenga og kan brukes hver dag kl. 08:00-00:00. Nøkkel eller adgang bestilles ved å kontakte styret. Den registrerte ordningen har en avgift på 300 kroner via Vipps og gjelder så lenge medlemskapet er aktivt.

Kontroller alltid pris og gjeldende adgangsrutine med styret før betaling. La hytta være ryddig, sikre dører og vinduer, og følg oppslagene på stedet.

[Button: Les om klubbhytta]({{clubhouse_url}})

## Informasjon og miljø

:::columns
:::column
## Medlemsgruppen

[Facebook-gruppen for medlemmer]({{facebook_members_url}}) brukes til klubbaktiviteter, praktiske beskjeder og diskusjoner mellom medlemmene.

:::column
## Offentlig gruppe

[Den offentlige Facebook-gruppen]({{facebook_market_url}}) er åpen for kjøp og salg, spørsmål og generell hobbyprat.
:::endcolumns

Følg også bodomfk.no for regler, kontaktinformasjon, værstasjon og webkamera. Viktige sikkerhetsbeskjeder bør leses før neste flytur.

## Hendelser, uhell og læring

Ved akutt fare eller personskade skal nødetatene varsles først. Sikre området, følg klubbens handlingsplan og varsle klubbens ansvarlige så snart det er mulig.

Hendelser, ulykker og observasjoner rapporteres i NLFs OBSREG-system. Rapportering handler om å lære og forebygge, ikke om å plassere skyld. OBSREG bruker samme idrettsinnlogging som Min idrett og TMS.

[Button: Meld hendelse eller observasjon]({{incident_url}})

## Støtt klubben med Grasrotandelen

Du kan velge Bodø Modellflyklubb som grasrotmottaker hos [Norsk Tipping](https://www.norsk-tipping.no/grasrotandelen). Søk på klubbnavnet eller organisasjonsnummer **993 764 299**. Det koster deg ikke noe ekstra, men gir klubben midler til anlegg, sikkerhet og utstyr.

## Hvem spør jeg?

For spørsmål om flyplassen, klubbhytta, opplæring eller klubbmiljøet kontakter du Bodø Modellflyklubb på {{contact_email_link}}. Du kan også bruke [kontaktsiden]({{contact_url}}) eller medlemsgruppen på Facebook.

For spørsmål om Min idrett, NLF-faktura, operatørregistrering eller kompetansebevis bruker du [NLFs kontaktinformasjon for modellflyseksjonen](https://nlf.no/grener/modellfly/modellflyseksjonen/kontakt-modellflyseksjonen/).

> Denne guiden ble oppdatert 23. juli 2026. Regler, systemer og priser kan endres. Nettsiden, klubbens oppslag og NLFs offisielle sider er alltid gjeldende kilder.
