...
Dette er rutinen for versjon 1.5.1 og nyere. Den tidligere migreringsutvidelsen er ferdig brukt og skal ikke installeres på nytt.
...
Filen skal hete omtrent `bodomfk-modern-theme-1.5.1.zip`. Ikke last opp «Source code (zip)»; den inneholder hele GitHub-prosjektet og kan ikke installeres direkte som tema.

## Oppdater i WordPress

1. Gå til **Utseende → Temaer → Legg til nytt tema → Last opp tema**.
2. Velg `bodomfk-modern-theme-1.5.1.zip`.
3. Trykk **Installer nå**.
4. Når WordPress finner den gamle versjonen, velg **Erstatt gjeldende med opplastet**.
5. Kontroller at **Bodø Modellflyklubb Modern 1.5.1** fortsatt er aktivt.
